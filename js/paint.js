...
function fitCanvasToImage() {
    ...
    if (imgRatio > canvasRatio) {
        ...
        offsetY = (CANVAS_HEIGHT - drawHeight) / 2; // 垂直居中
    } else {
        // 图片更高或比例相同，按高度缩放，可能会裁剪宽度
        drawHeight = CANVAS_HEIGHT;
        drawWidth = CANVAS_HEIGHT * imgRatio;
        offsetX = (CANVAS_WIDTH - drawWidth) / 2; // 水平居中
    }
    ...
    ctx.drawImage(
        originalImage, 
        0, 0, imgWidth, imgHeight,  // 原始图片
        offsetX, offsetY, drawWidth, drawHeight  // 缩放后绘制
    );

    // 更新画布标题
    const canvasTitle = document.querySelector('.canvas-title');
    canvasTitle.textContent = `预览画布 (800x480) - 图片已铺满`;

    // 日志提示
    logMessage(`图片已按比例铺满画布 (${CANVAS_WIDTH}x${CANVAS_HEIGHT})`);
}
